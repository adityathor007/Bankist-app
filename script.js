...
const labelDate = document.querySelector('.dating');
...
const calcDisplayBalance = function (acc) {
  ...
  labelBalance.textContent =
    formatCur(acc.balance, acc.locale,
      acc.currency);
};
...
const calcDisplaySummary = function (acc) {
  ...
  labelSumOut.textContent =
    formatCur(Math.abs(out), acc.locale,
      acc.currency);
...
btnLoan.addEventListener('click', function (e) {
  ...
  if (amount > 0 && currentAccount.movements.some(
    ...
    setTimeout(function () {
      ...
      // Add loan date
      currentAccount.movementsDates.push(new
        Date().toISOString());
